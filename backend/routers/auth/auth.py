...
async def get_current_user(
    ...
    supabase_user = auth_helpers.verify_token(token)    # Get user profile from our database using user_id (which references auth.users.id)
...
async def register(
    ...
    try:
        ...
        new_user_profile = UserProfile(
            user_id=supabase_user_id,  # Reference to auth.users.id
            username=user_data.username,
            first_name=user_data.first_name,
            last_name=user_data.last_name
        )
...
async def login(
    ...
    try:
        ...
        if auth_response.user is None or auth_response.session is None:
            ...
            )        # Get user profile from our database using user_id
        result = await db.execute(
